...
// logger 参数用于日志记录。
func NewHandler(config *platform.PlatformConfig /*, logger client.Logger */) (*VolcengineHandler, error) {
	if config == nil {
	...
	return &VolcengineHandler{
		apiKey:     apiKey,
		httpClient: &http.Client{}, // 使用默认的 http.Client，后续可以配置超时等
		// logger:     logger,
	}, nil
}
